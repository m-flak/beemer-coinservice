...
public class StandardMerkleTree {
	...
	public static StandardMerkleTree of(List<byte[]> leafHashes) {
		byte[][] sorted = leafHashes.stream().map(byte[]::clone).sorted(StandardMerkleTree::compareBytes)
				.toArray(byte[][]::new);
